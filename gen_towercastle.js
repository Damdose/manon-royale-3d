#!/usr/bin/env node
/* Génère les CHÂTEAUX VIDES (sans perso) en image, style Clash Royale, vue 3/4 plongée.
   Le perso animé (towerchar_cut_*) sera posé par-dessus dans le jeu.
     KIE_API_KEY=xxxx node gen_towercastle.js [cle...]   (sans arg = les 4)
   -> assets/towercastle_<role>_<side>.png  puis :
      .venv-rembg/bin/python gen_troops.py --towercastle */
...
const STYLE="Official Clash Royale arena tower by Supercell, stylized 3D-rendered, exact Supercell mobile-game look: "
 ...
 +"The battlement on top is EMPTY: NO character, NO person, NO soldier, NO cannon — just the empty stone platform inside the crenellations. "
...
const PROMPTS={
  king_blue:"a large imposing empty KING tower; BLUE banners and blue pointed turret roofs; a golden crown emblem on the front wall; empty battlement on top",
  king_red:"a large imposing empty KING tower; RED banners and red pointed turret roofs; a golden crown emblem on the front wall; empty battlement on top",
  princess_blue:"a smaller empty PRINCESS archer tower; BLUE accents and blue pointed turret roofs; a wooden ladder on the front; empty battlement on top",
  princess_red:"a smaller empty PRINCESS archer tower; RED accents and red pointed turret roofs; a wooden ladder on the front; empty battlement on top",
};

const sleep=ms=>new Promise(r=>setTimeout(r,ms));
async function genOne(key){
  const out=path.join(ASSETS,'towercastle_'+key+'.png');
...
(async()=>{
  ...
  console.log('Génération de',keys.length,'château(x) vide(s) ->',ASSETS);
  let ok=0;for(const k of keys){if(!PROMPTS[k]){console.log('clé inconnue:',k);continue;}if(await genOne(k))ok++;}
  console.log(`\nTerminé : ${ok}/${keys.length}. Détoure :  .venv-rembg/bin/python gen_troops.py --towercastle`);
})();
